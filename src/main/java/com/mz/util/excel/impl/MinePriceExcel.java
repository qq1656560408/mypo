...
public class MinePriceExcel extends BaseExcelDataProcessing {
	...
	public MinePriceExcel(IWaterTypeandFeaturesTableService typeAndFeaturesService, InitBaseData initBaseData) {
	...
	List<MinePriceTable> priceTables = new ArrayList<>();

	public List<MinePriceTable> getPriceTables() {
		return priceTables;
	}

	public void setPriceTables(List<MinePriceTable> priceTables) {
	...
	public void setParam(int yearID, int typeID, String features, String areaName, int cellIndex, Workbook wookbook,
		...
		// 价格表参数
		MinePriceTable minePriceTable = new MinePriceTable();
		minePriceTable.setTypeAndFeaturesID(waterTypeandFeaturesTable.getWaterTypeAndFeaturesID());
		minePriceTable.setPrice(value);
		minePriceTable.setUnit(utilStr);
		minePriceTable.setRemark(remarkStr);
		minePriceTable.setCoefficient(coefficientDou);
		priceTables.add(minePriceTable);
	...
	public void handData(boolean end, Integer rowIndex, Workbook wookbook, Row row) {
		 ...
		 */
		double coefficientDou = 0;
		// 备注
		/*
		 * Cell remark = row.getCell(15); String remarkStr = getString(remark);
		 */
		String remarkStr = "";
