...
public class RenewablePriceExcel extends BaseExcelDataProcessing {
	...
	public RenewablePriceExcel(IWaterTypeandFeaturesTableService typeAndFeaturesService, InitBaseData initBaseData) {
	...
	List<RenewablePriceTable> renewablePriceTables = new ArrayList<>();

	public List<RenewablePriceTable> getRenewablePriceTables() {
		return renewablePriceTables;
	}

	public void setRenewablePriceTables(List<RenewablePriceTable> renewablePriceTables) {
		this.renewablePriceTables = renewablePriceTables;
	}
	...
	public void setParam(int yearID, int typeID, String features, String areaName, int cellIndex, Workbook wookbook,
		...
		// 价格表参数
		RenewablePriceTable renewablePriceTable = new RenewablePriceTable();
		renewablePriceTable.setTypeAndFeaturesID(waterTypeandFeaturesTable.getWaterTypeAndFeaturesID());
		renewablePriceTable.setPrice(value);
		renewablePriceTable.setUnit(utilStr);
		renewablePriceTable.setRemark(remarkStr);
		renewablePriceTable.setCoefficient(coefficientDou);
		renewablePriceTables.add(renewablePriceTable);
	...
	public void handData(boolean end, Integer rowIndex, Workbook wookbook, Row row) {
		...
		// 系数
		/*
		 * Cell coefficient = row.getCell(14); Double coefficientDou =
		 * getDouble(wookbook, coefficient);
		 */
