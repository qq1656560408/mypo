...
<body>
		...
		<div style="height: 500px; width: 920px; padding-top: 10px;">
			...
			<div style="width: 700px; height: 100%; overflow: hidden; border: 1px solid #ccc;">
				<div>
					...
					<iframe id="showPdf" style="height: 430px; margin: auto; display: block; width: 100%">
					</iframe>
	...
	<script>
		layui.use('upload', function() {
			...
			waterUpload.render({
				elem : '#file',
				url : '${path}/processFileTable/updateFile.do',
				auto : false,
				bindAction : '#begin',
				accept : 'file',
				exts : "pdf",
				...
				before : function(obj) {
					...
					if (res.path == "") {
						...
						$("#showPdf").attr(
								"src",
								"${path}/processFileTable/lookPdf.do?pdfPath="
										+ res.path);
						var tree = $('#container').jstree(true);
						var node = tree.get_selected(true)[0];
						$(node)[0].original.processFileURL = res.path;
					}
		...
		$(function() {
			...
			function deleteData(inst, node) {
				$.ajax({
					url : "${path}/processFileTable/deleteRemoteSensing.do",
					data : {
						id : node.id
					},
					dataType : "json",
			...
			$('#container').on("select_node.jstree", function(e, data) {
				var node = data.node;
				//最低级节点
				var url = node.original.processFileURL;
				//查看影像
			...
			$('#container').on("rename_node.jstree", function(e, data) {
				if (data.old != data.text) {
					...
					//说明节点更新
					$.ajax({
						url : "${path}/processFileTable/updateName.do",
						data : {
							name : data.text,
							id : id
						},
			...
			$('#container').on("move_node.jstree", function(e, data) {
				...
				var id = data.node.id;
				$.ajax({
					url : "${path}/processFileTable/updateParentsID.do",
					data : {
						parentsID : parentsID,
						id : id
					},
			...
			function saveNode(node, name, paranID) {
				$.ajax({
					url : "${path}/processFileTable/insertTopNode.do",
					data : {
						name : name,
						paranID : paranID
					},
			...
			function look(url) {
				$("#showPdf").attr("src",
						"${path}/processFileTable/lookPdf.do?pdfPath=" + url);
			}
