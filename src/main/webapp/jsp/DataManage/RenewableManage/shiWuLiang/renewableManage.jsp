...
<body>
	<div style="padding: 10px; width: 80%; margin: auto;">
		<div class="layui-upload" style="display: block;">
		    <button  type="button" class="layui-btn" id="insertRenewableShiWu">新增</button>
			<button type="button" class="layui-btn layui-btn-normal" id="renewableFile">选择文件</button>
			<button type="button" class="layui-btn" id="beginRenewableUpload">开始上传</button>
		</div>
	...
	<script type="text/javascript">
		...
		layui.use('upload', function() {
			...
			uploadRenewable.render({
				elem : '#renewableFile',
				url : '${path}/renewablePhyQuantityTable/excelImprot.do',
				auto : false,
				bindAction : '#beginRenewableUpload',
				accept : 'file',
				exts : "xlsx|xls",
				field : "excelFile",
				done : function(res) {
		...
		function initRenewableTable(){
			showRenewableData.init('showRenewableData', {
				height : 480,//设置高度
				page : true,
				url : '${path}/renewablePhyQuantityTable/selectPhyQuantity.do'
			});
		}
		var tableRenewable ;
		layui.use('table', function() {
			showRenewableData= layui.table;
			initRenewableTable();
		
			showRenewableData.on('tool(showRenewableData)', function(obj) {
				var contentStr,title;
				if(obj.event=="update"){
					var id=obj.data.renewableID;
					contentStr=["${path}/renewablePhyQuantityTable/bingUpdateBaseData.do?id="+id,"no"];
					title="修改实物量";
		...
		function deleteRenewableData(obj){
			var id=obj.data.renewableID;
			$.ajax({
				url:"${path}/renewablePhyQuantityTable/deletePhyQuantity.do",
				data:{id:id},
				dataType:"json",
				type:"post",
		...
		$("#insertRenewableShiWu").click(function(){
			contentStr=["${path}/renewablePhyQuantityTable/bingInsertBaseData.do","no"];
			title="新增实物量";
			showRenewableLayer(contentStr,title);
		});
