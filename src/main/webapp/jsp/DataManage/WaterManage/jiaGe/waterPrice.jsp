...
<body>
	<div style="width: 80%; margin: auto;">
		<div class="layui-upload" style="display: block;">
			...
			<button type="button" class="layui-btn layui-btn-normal" id="waterExcelFile">选择文件</button>
			<button type="button" class="layui-btn" id="beginWaterUpload">开始上传</button>
		</div>
			...
			<table class="layui-hide" lay-filter="showWaterData" lay-data="page:true,limit:10">
				<thead>
					<tr>
						...
						<th lay-data="{field:'edit',width:170,toolbar: '#editBarWater'}">编辑/删除</th>
					</tr>
				</thead>
			</table>
	...
	<script type="text/javascript">
		...
		layui.use('upload', function() {
			...
			waterUpload.render({
				elem : '#waterExcelFile',
				url : '${path}/waterPriceTable/excelImprot.do',
				auto : false,
				bindAction : '#beginWaterUpload',
				accept : 'file',
				exts : "xlsx|xls",
				field : "excelFile",
				done : function(res) {
		...
		function initWaterTable(){
			waterTable.init('showWaterData', {
				height : 480,//设置高度
				page : true,
				url : '${path}/waterPriceTable/selectPriceTable.do'
			});
		}
		var waterTable;
		layui.use('table', function() {
			...
			waterTable.on('tool(showWaterData)', function(obj) {
			    ...
			    if(obj.event=="update"){
			    	var id=obj.data.waterPriceID;
					contentStr=["${path}/waterPriceTable/bingUpdateBaseData.do?id="+id,"no"];
					title="修改价格";
					showWaterLayer(contentStr,title);
		...
		function deleteWaterData(obj){
			var id=obj.data.waterPriceID;
			$.ajax({
				url:"${path}/waterPriceTable/deletePrice.do",
				data:{id:id},
				dataType:"json",
				type:"post",
		...
		$("#insertWaterShiWu").click(function(){
			contentStr=["${path}/waterPriceTable/bingInsertBaseData.do","no"];
			title="新增价格";
			showWaterLayer(contentStr,title);
		});
