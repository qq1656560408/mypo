...
<body>
	<form class="layui-form" style="padding: 10px;">
		<input name="forestID" type="hidden" value="${spq.forestID}">
		<input name="typeAndFeaturesID" type="hidden" value="${spq.typeAndFeaturesID}">
		<div class="layui-form-item">
			<div class="layui-inline">
				<label class="layui-form-label">年份</label>
				<div class="layui-input-inline">
					<input type="text" name="year" class="layui-input" id="forestYear" value="${spq.yearMC}"
						readonly="readonly">
				</div>
			</div>
			<div class="layui-inline">
		...
		<div class="layui-form-item">
			<div class="layui-input-block">
				<button class="layui-btn" lay-submit lay-filter="forestSub">立即提交</button>
			</div>
		</div>
	</form>
	<script type="text/javascript">
		...
		layui.use('form', function() {
			...
			form.on('submit(forestSub)', function(getData) {
				var data = getData.field; //当前容器的全部表单字段，名值对形式：{name: value}
				$.ajax({
					url : "${path}/forestPhyQuantityTable/updatePhyQuantity.do",
					data : data,
					dataType : "json",
					type : "post",
